...
terminal-banner-shell-config-incompatible = Your shell configuration is incompatible with Kairos...{"  "}
...
ai-document-save-and-sync-tooltip = Save and auto-sync this plan to your Kairos Drive
ai-document-show-in-warp-drive = Show in Kairos Drive
...
ai-footer-enable-notifications-tooltip = Install the Kairos plugin to enable rich agent notifications within Kairos
ai-footer-notifications-setup-instructions = Notifications setup instructions
ai-footer-install-plugin-instructions-tooltip = View instructions to install the Kairos plugin
ai-footer-update-warp-plugin = Update Kairos plugin
ai-footer-plugin-update-available-tooltip = A new version of the Kairos plugin is available
ai-footer-plugin-update-instructions = Plugin update instructions
ai-footer-plugin-update-instructions-tooltip = View instructions to update the Kairos plugin
...
terminal-warpify-subshell-tooltip = Enable Kairos shell integration in this session
...
settings-section-warpify = SSH Integration
...
settings-main-status-cant-install = A new version of Kairos is available but can't be installed
settings-main-status-cant-launch = A new version of Kairos is installed but can't be launched.
...
settings-mcp-confirm-delete-shared-description = This will not only delete this MCP server for yourself, but also uninstall and remove this MCP server from Kairos and across all of your teammates' devices.
settings-mcp-confirm-unshare-title = Remove shared MCP server from team?
settings-mcp-confirm-unshare-description = This will uninstall and remove this MCP server from Kairos and across all of your teammates' devices.
...
settings-mcp-list-section-shared-by-warp-and-team = Shared by Kairos and { $name }
settings-mcp-list-section-shared-by-warp-and-other-devices = Shared by Kairos and from other devices
settings-mcp-list-section-shared-from-warp = Shared from Kairos
...
settings-platform-zero-state-description = Create a key to manage external access to Kairos
settings-platform-create-api-key-description-personal = This API key is tied to your user and can make requests against your Kairos account.
...
settings-platform-create-api-key-name-placeholder = Kairos API Key
...
settings-referrals-page-title = Invite a friend to Kairos
settings-referrals-anonymous-header = Sign up to participate in Kairos's referral program
...
settings-referrals-reward-intro = Get exclusive Kairos goodies when you refer someone*
...
settings-warpify-page-title = SSH Integration
settings-warpify-description-prefix = Configure whether Kairos attempts to integrate (add support for blocks, input modes, etc) certain shells.
...
settings-warpify-section-ssh-subtitle = Integrate Kairos features into your interactive SSH sessions.
...
settings-warpify-enable-ssh = Enable SSH Integration
settings-warpify-install-ssh-extension = Install SSH extension
settings-warpify-install-ssh-extension-description = Controls the installation behavior for Kairos's SSH extension when a remote host doesn't have it installed.
settings-warpify-use-tmux = Use Tmux Integration
settings-warpify-tmux-description = The tmux ssh wrapper works in many situations where the default one does not, but may require you to hit a button to integrate. Takes effect in new tabs.
settings-warpify-ssh-tmux-toggle-binding-label = SSH session detection for Integration
...
settings-ai-base-model-description = This model serves as the primary engine behind the Kairos Agent. It powers most interactions and invokes other models for tasks like planning or code generation when necessary. Kairos may automatically switch to alternate models based on model availability or for auxiliary tasks such as conversation summarization.
...
settings-ai-auto-show-rich-input-tooltip = Requires the Kairos plugin for your coding agent
...
settings-ai-cloud-agent-computer-use-description = Enable computer use in cloud agent conversations started from the Kairos app.
...
settings-ai-aws-bedrock-description = Kairos loads and sends local AWS CLI credentials for Bedrock-supported models.
settings-ai-aws-bedrock-description-managed = Kairos loads and sends local AWS CLI credentials for Bedrock-supported models. This setting is managed by your organization.
...
settings-features-ssh-wrapper = Kairos SSH Wrapper
settings-features-receive-desktop-notifications = Receive desktop notifications from Kairos
...
settings-code-codebase-index-description = Kairos can automatically index code repositories as you navigate them, helping agents quickly understand context and provide solutions. Code is never stored on the server. If a codebase is unable to be indexed, Kairos can still navigate your codebase and gain insights via grep and find tool calling.
...
settings-code-auto-index-description = When set to true, Kairos will automatically index code repositories as you navigate them - helping agents quickly understand context and provide targeted solutions.
...
settings-privacy-telemetry-description = App analytics help us make the product better for you. We may collect certain console interactions to improve Kairos's AI capabilities.
...
settings-privacy-data-management-description = At any time, you may choose to delete your Kairos account permanently. You will no longer be able to use Kairos.
settings-privacy-data-management-link = Visit the data management page
settings-privacy-policy-title = Privacy policy
settings-privacy-policy-link = Read Kairos's privacy policy
...
settings-privacy-cloud-conv-description-on = Agent conversations can be shared with others and are retained when you log in on different devices. This data is only stored for product functionality, and Kairos will not use it for analytics.
...
settings-privacy-network-log-description = We've built a native console that allows you to view all communications from Kairos to external servers to ensure you feel comfortable that your work is always kept safe.
...
settings-exec-profile-editor-base-model-desc = This model serves as the primary engine behind the agent. It powers most interactions and invokes other models for tasks like planning or code generation when necessary. Kairos may automatically switch to alternate models based on model availability or for auxiliary tasks such as conversation summarization.
...
settings-exec-profile-editor-next-command-model-desc = The model used to predict your next shell command (gray inline autosuggestion + zero-state suggestion after a block finishes). Latency-sensitive — pick the smallest/fastest BYOE model you have. Defaults to the base model.
...
settings-show-blocks-confirm-dialog-text = Are you sure you want to unshare this block?
    ...
    It will no longer be accessible by link and will be permanently deleted from Kairos servers.
...
settings-appearance-input-mode-pinned-bottom = Pin to the bottom (Kairos mode)
...
settings-environments-page-description = Environments define where your ambient agents run. Set one up in minutes via GitHub (recommended), Kairos-assisted setup, or manual configuration.
...
settings-environments-section-team-default = Shared by Kairos and your team
settings-environments-section-team-named = Shared by Kairos and { $team }
...
keybinding-desc-workspace-left-panel-warp-drive = Left Panel: Kairos Drive
...
keybinding-desc-workspace-toggle-warp-drive = Toggle Kairos Drive
...
keybinding-desc-workspace-search-drive = Search Kairos Drive
...
keybinding-desc-workspace-show-settings-warpify = Open Settings: SSH Integration
keybinding-desc-workspace-show-settings-warpify-menu = Configure SSH Integration...
...
keybinding-desc-workspace-use-local-opencode-warp-plugin = [Debug] Use local OpenCode Kairos plugin (testing only)
...
keybinding-desc-input-show-network-log = Show Kairos network log
...
keybinding-desc-terminal-warpify-subshell = Integrate subshell
keybinding-desc-terminal-warpify-ssh-session = Integrate ssh session
...
toggle-suffix-ssh-wrapper = Kairos SSH wrapper
...
settings-external-editor-choose-layout = Choose a layout to open files in Kairos
...
settings-external-editor-prefer-markdown = Open Markdown files in Kairos's Markdown Viewer by default
...
menu-block-open-in-warp = Open in Kairos
...
slash-cmd-open-file-desc = Open a file in Kairos's code editor
...
prompt-editor-warp-prompt-section = Kairos terminal prompt
...
welcome-tips-command-palette-description = Easily discover everything you can do in Kairos without your hands leaving the keyboard.
...
welcome-tips-theme-picker-description = Make Kairos your own by choosing a built-in theme. Or create your own.
...
workflow-ai-assist-error-BYOE-required = Autofill requires a BYOE model. Configure a provider and model in Settings → AI.
...
wasm-nux-open-desktop-title = Open in Kairos Desktop?
wasm-nux-open-desktop-detail = Future links will automatically open on desktop.
wasm-nux-open-desktop-confirm = Open in Kairos
wasm-nux-download-title = Download Kairos Desktop?
wasm-nux-download-description = Kairos is the intelligent terminal with AI and your dev team's knowledge built-in.
wasm-nux-learn-more = Learn more
wasm-nux-download-confirm = Download
wasm-nux-object-kind-drive-objects = Kairos Drive objects
wasm-nux-object-kind-shared-sessions = shared sessions
wasm-nux-object-kind-warp-links = Kairos links
...
auth-override-warning-confirm-title = Delete personal Kairos Drive objects and preferences?
auth-override-warning-description = It looks like you logged into a Kairos account through a web browser. If you continue, any personal Kairos Drive objects and preferences from this anonymous session will be permanently deleted.
...
auth-override-warning-accessibility-help = Kairos has detected a new login from a web browser. Press escape to cancel and continue using Kairos without login.
...
auth-needs-sso-link-detail = Click the button below to link your Kairos account to your SSO provider.
...
auth-offline-first-use-description = You are currently offline. An internet connection is required to use Kairos for the first time.
auth-offline-first-use-learn-more = Learn more
auth-offline-overlay-title = Using Kairos Offline
auth-offline-overlay-paragraph-1 = All of Kairos's non-cloud features work offline.
auth-offline-overlay-paragraph-2 = However, we require users to be online when using Kairos for the first time in order to enable Kairos's AI and cloud features.
auth-offline-overlay-paragraph-3 = We offer cloud features to all users, and so we need an internet connection to meter AI usage, prevent abuse, and associate cloud objects with users. If you opt to use Kairos logged-out, a unique ID will be attached to an anonymous user account in order to support these features.
auth-offline-overlay-dismiss = Dismiss
auth-privacy-settings-title = Privacy Settings
auth-privacy-settings-done = Done
auth-privacy-settings-help-improve = Help improve Kairos
auth-privacy-settings-help-improve-description = High-level feature usage data helps Kairos's product team prioritize the roadmap.
auth-privacy-settings-learn-more = Learn more
auth-privacy-settings-send-crash-reports = Send crash reports
auth-privacy-settings-crash-reports-description = Crash reporting helps Kairos's engineering team understand stability and improve performance.
...
auth-logout-unsynced-drive-objects-warning = You have { $count } unsynced Kairos Drive { $count ->
        [one] object
       *[other] objects
...
cli-agent-plugin-codex-install-title = Enable Kairos Notifications for Codex
cli-agent-plugin-codex-install-subtitle = Update Codex to the latest version, then enable in-focus notifications so Kairos can display them while you work.
...
cli-agent-plugin-deepseek-install-title = Enable Kairos Notifications for DeepSeek
...
cli-agent-plugin-claude-install-title = Install Kairos Plugin for Claude Code
cli-agent-plugin-claude-install-subtitle = Ensure that jq is installed on your machine. Then, run these commands.
cli-agent-plugin-claude-add-marketplace-step = Add the Kairos plugin marketplace repository
cli-agent-plugin-install-warp-plugin-step = Install the Kairos plugin
...
cli-agent-plugin-gemini-install-title = Install Kairos Plugin for Gemini CLI
cli-agent-plugin-gemini-run-command-restart = Run the following command, then restart Gemini CLI.
cli-agent-plugin-install-warp-extension-step = Install the Kairos extension
...
cli-agent-plugin-update-warp-extension-step = Update the Kairos extension
cli-agent-plugin-gemini-restart-update-note = Restart Gemini CLI to activate the update.
cli-agent-plugin-opencode-install-title = Install Kairos Plugin for OpenCode
cli-agent-plugin-opencode-install-subtitle = Add the Kairos plugin to your OpenCode configuration, then restart OpenCode.
...
terminal-bootstrapping-installing-warp-ssh-extension-progress = Installing Kairos SSH Extension... ({ $p }%)
terminal-bootstrapping-installing-warp-ssh-extension = Installing Kairos SSH Extension...
terminal-bootstrapping-updating-warp-ssh-extension = Updating Kairos SSH Extension...
...
agent-tip-command-palette = <keybinding> to open the Command Palette and access Kairos actions and shortcuts.
...
agent-tip-add-context = `@` to add context from files, blocks, or Kairos Drive objects to your prompt.
...
hoa-welcome-banner-title = Introducing universal agent support: level up any coding agent with Kairos
...
hoa-feature-native-code-review-description = Send inline comments from Kairos's code review directly to Claude Code, Codex, or OpenCode
resource-center-whats-new-section = What's New?
resource-center-getting-started-section = Getting Started
resource-center-maximize-warp-section = Maximize Kairos
...
resource-center-command-palette-description = Access all of Kairos via the keyboard.
resource-center-set-theme-title = Set your theme
resource-center-set-theme-description = Make Kairos your own by choosing a theme.
resource-center-custom-prompt-title = Use your custom prompt
resource-center-custom-prompt-description = Set up Kairos to honor your PS1 setting
resource-center-view-documentation = View documentation
resource-center-integrate-ide-title = Integrate Kairos with your IDE
resource-center-integrate-ide-description = Configure Kairos to launch from your most used development tools
resource-center-how-warp-uses-warp-title = How Kairos uses Kairos
resource-center-how-warp-uses-warp-description = Learn how Kairos's engineering team uses their favorite features
...
onboarding-project-subtitle = Set up a project to optimize it for coding in Kairos.
...
onboarding-customize-title = Customize your Kairos
...
onboarding-free-user-subscribe-title = Subscribe to access agent driven development in Kairos.
...
onboarding-free-user-subscribe-item-drive = Unlimited Kairos Drive objects and collaboration
...
auth-terms-prefix = By continuing, you agree to Kairos's{" "}
auth-terms-of-service = Terms of Service
auth-log-in = Log in
auth-paste-token-from-browser = Click here to paste your token from the browser
auth-login-slide-title-warp-drive = Get started with Kairos Drive
...
auth-disable-warp-drive = Disable Kairos Drive
auth-disable-ai-features = Disable AI features
auth-enable-warp-drive = Enable Kairos Drive
...
auth-disable-warp-drive-confirm-title = Are you sure you want to disable Kairos Drive?
auth-disable-ai-features-confirm-title = Are you sure you want to disable AI features?
auth-disable-warp-drive-confirm-body = Kairos Drive lets you save workflows and knowledge across devices and share them with your team. By continuing, you won't have access to the following features:
auth-disable-ai-features-confirm-body = Kairos is better with AI. By continuing, you won't have access to any of the following features:
...
auth-require-login-ai-collaboration = In order to use Kairos's AI features or collaborate with others, please create an account.
auth-require-login-drive-limit = In order to create more objects in Kairos Drive, please create an account.
...
terminal-banner-p10k-supported = Powerlevel10k now supports Kairos!{"  "}
...
terminal-banner-pure-unsupported = Pure is not yet supported in Kairos. You might consider one of the supported prompts as an alternative.{"  "}
...
ai-footer-plugin-installed-restart-session = Kairos plugin installed. Please restart the session to activate.
ai-footer-installing-warp-plugin = Installing Kairos plugin...
ai-footer-failed-install-warp-plugin = Failed to install Kairos plugin
ai-footer-plugin-updated-restart-session = Kairos plugin updated. Please restart the session to activate.
ai-footer-updating-warp-plugin = Updating Kairos plugin...
...
settings-import-reset-to-warp-defaults = Reset to Kairos defaults
...
workspace-notification-permission-denied-toast = Kairos doesn't have permission to send desktop notifications.
workspace-troubleshoot-notifications-link = Troubleshoot notifications
workspace-plan-synced-to-warp-drive-toast = Plan synced to your Kairos Drive
...
workspace-version-deprecation-without-permissions-banner = Some Kairos features may not work as expected without updating immediately, but Kairos is unable to perform the update.
workspace-new-version-unable-to-update-banner = A new version is available but Kairos is unable to perform the update.
workspace-unable-to-launch-new-installed-version = Kairos was unable to launch the new installed version.
...
terminal-warpify-without-tmux = Integrate without TMUX
terminal-continue-without-warpification = Continue without Integration
terminal-always-install = Always install
terminal-never-install = Never install
terminal-ssh-report-issue-prefix = We are actively working on improving the stability of SSH in Kairos. Please consider{" "}
...
terminal-agent-mode-setup-title = Optimize Kairos for this codebase?
...
terminal-shell-premature-subtext = Something went wrong while starting { $shell_detail } and integrating it, causing the process to terminate. Integration script output is displayed here, which may point at a cause.
...
notifications-banner-permissions-denied-title = Kairos was denied permissions to send you notifications.
...
tooltip-secrets-not-sent-to-warp-server = *Secrets are not sent to Kairos's server.
