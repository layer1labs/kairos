...
common-skip = 跳过
common-get-warping = 开始使用 Kairos
common-try-again = 重试
common-settings = 设置
common-premium = 高级版
...
agent-management-source-local-agent = Kairos（本地智能体）
agent-management-source-cloud-agent = Kairos（云端智能体）
...
workspace-menu-update-warp-manually = 手动更新 Kairos
...
workspace-menu-view-warp-logs = 查看 Kairos 日志
...
workspace-update-and-relaunch-warp = 更新并重启 Kairos
workspace-updating-to-version = 正在更新到 ({ $version })
workspace-update-warp-manually = 手动更新 Kairos
...
terminal-banner-shell-config-incompatible = 你的 shell 配置与 Kairos 不兼容...{"  "}
...
ai-document-save-and-sync-tooltip = 保存并将此计划自动同步到你的 Kairos Drive
ai-document-show-in-warp-drive = 在 Kairos Drive 中查看
...
ai-footer-enable-notifications-tooltip = 安装 Kairos 插件以在 Kairos 中启用丰富的智能体通知
ai-footer-notifications-setup-instructions = 通知设置说明
ai-footer-install-plugin-instructions-tooltip = 查看安装 Kairos 插件的说明
ai-footer-update-warp-plugin = 更新 Kairos 插件
ai-footer-plugin-update-available-tooltip = Kairos 插件有新版本可用
ai-footer-plugin-update-instructions = 插件更新说明
ai-footer-plugin-update-instructions-tooltip = 查看更新 Kairos 插件的说明
...
openwarp-launch-title = Kairos 现已开源
openwarp-launch-description = 你和我们的社区现在可以使用智能体优先的工作流参与构建 Kairos。
openwarp-launch-contribute-title = 参与贡献
openwarp-launch-contribute-description = Kairos 客户端代码现已开源。你可以先使用 /feedback skill 创建 issue，并阅读这里的贡献指南。
openwarp-launch-contribute-link-text = 这里
openwarp-launch-oad-title = 开放自动化开发
openwarp-launch-oad-description = Kairos 仓库由智能体优先的工作流管理，并由我们的云端智能体编排平台 Oz 提供支持。
...
hoa-finish = 完成
session-config-get-warping = 开始使用 Kairos
...
terminal-warpify-subshell-tooltip = 在此会话中启用 Kairos shell 集成
terminal-use-agent = 使用智能体
terminal-use-agent-tooltip = 让 Kairos 智能体协助
terminal-give-control-back-to-agent = 将控制权交还给智能体
terminal-resume-agent-tooltip = 让 Kairos 智能体继续
...
terminal-open-in-warp = 在 Kairos 中打开
terminal-open-conversation-in-warp-tooltip = 在 Kairos 桌面端打开此对话
...
terminal-input-agent-hint-deploy-react-vercel = 让 Kairos 处理任何任务，例如将 React 应用部署到 Vercel 并配置环境变量
terminal-input-agent-hint-debug-python-ci = 让 Kairos 处理任何任务，例如帮我调试 Python 测试在 CI 中失败的原因
terminal-input-agent-hint-setup-microservice = 让 Kairos 处理任何任务，例如用 Docker 搭建新的微服务并创建部署流水线
terminal-input-agent-hint-fix-node-memory-leak = 让 Kairos 处理任何任务，例如查找并修复 Node.js 应用中的内存泄漏
terminal-input-agent-hint-backup-postgres = 让 Kairos 处理任何任务，例如为 PostgreSQL 数据库创建备份脚本并设置定时任务
terminal-input-agent-hint-migrate-mysql-postgres = 让 Kairos 处理任何任务，例如帮我把数据从 MySQL 迁移到 PostgreSQL
terminal-input-agent-hint-monitor-aws = 让 Kairos 处理任何任务，例如为 AWS 基础设施设置监控和告警
terminal-input-agent-hint-build-fastapi = 让 Kairos 处理任何任务，例如用 FastAPI 为移动应用构建 REST API
terminal-input-agent-hint-optimize-sql = 让 Kairos 处理任何任务，例如帮我优化运行缓慢的 SQL 查询
terminal-input-agent-hint-github-actions = 让 Kairos 处理任何任务，例如创建 GitHub Actions 工作流，在合并后自动部署
terminal-input-agent-hint-redis-cache = 让 Kairos 处理任何任务，例如为 Web 应用设置 Redis 缓存
terminal-input-agent-hint-kubernetes-pods = 让 Kairos 处理任何任务，例如帮我排查 Kubernetes Pod 持续崩溃的原因
terminal-input-agent-hint-bigquery-pipeline = 让 Kairos 处理任何任务，例如构建数据流水线来处理 CSV 文件并加载到 BigQuery
terminal-input-agent-hint-ssl-https = 让 Kairos 处理任何任务，例如设置 SSL 证书并为域名配置 HTTPS
terminal-input-agent-hint-refactor-legacy-code = 让 Kairos 处理任何任务，例如帮我将这段遗留代码重构为现代设计模式
terminal-input-agent-hint-unit-tests = 让 Kairos 处理任何任务，例如为认证服务创建单元测试
terminal-input-agent-hint-elk-logs = 让 Kairos 处理任何任务，例如为分布式系统设置 ELK 日志聚合
terminal-input-agent-hint-oauth-express = 让 Kairos 处理任何任务，例如帮我在 Express.js 应用中实现 OAuth2 认证
terminal-input-agent-hint-optimize-docker = 让 Kairos 处理任何任务，例如优化 Docker 镜像以减少构建时间和体积
terminal-input-agent-hint-ab-testing = 让 Kairos 处理任何任务，例如为 Web 应用搭建 A/B 测试基础设施
...
terminal-model-specs-description = Kairos 对模型在执行器中的表现、额度消耗速率和任务速度的基准测试。
...
onboarding-callout-meet-input-title = 认识 Kairos 输入框
...
onboarding-callout-nl-support-text-prefix = 默认情况下自然语言输入处于关闭状态。启用后，你可以直接用自然语言输入请求，Kairos 会为智能体自动识别查询。你也始终可以使用
onboarding-callout-enable-nl-detection = 启用自然语言识别
onboarding-callout-new-agent-title = 认识 Kairos 全新的智能体体验
...
language-widget-secondary = 重启 Kairos 以让此更改完全生效。
language-restart-required-title = 语言已切换
language-restart-required-body = Kairos 的界面语言已更新。部分文字会立即切换,完整生效需要重启 Kairos。
...
settings-section-warp-drive = Kairos Drive
settings-section-warpify = SSH 集成
settings-section-ai = AI
settings-section-warp-agent = Kairos 智能体
...
# about_page.rs
settings-about-copyright = 版权所有 2026 Kairos
...
settings-main-referral-cta = 与朋友和同事分享 Kairos,获得奖励
...
settings-main-cta-relaunch-warp = 重启 Kairos
settings-main-status-updating = 正在更新...
settings-main-status-installed-update = 更新已安装
settings-main-status-cant-install = 有新版本的 Kairos 可用,但无法安装
settings-main-status-cant-launch = 新版本的 Kairos 已安装,但无法启动。
settings-main-cta-update-manually = 手动更新 Kairos
...
settings-mcp-confirm-delete-shared-description = 此操作不仅会为你自己删除此 MCP 服务器,还会从 Kairos 以及所有团队成员的设备上卸载并移除此 MCP 服务器。
settings-mcp-confirm-unshare-title = 从团队中移除共享的 MCP 服务器?
settings-mcp-confirm-unshare-description = 这将从 Kairos 以及所有团队成员的设备上卸载并移除此 MCP 服务器。
...
# ---- list_page.rs ----
settings-mcp-list-description = 添加 MCP 服务器以扩展 Kairos Agent 的能力。MCP 服务器通过标准化接口向 agent 暴露数据源或工具,本质上类似插件。你可以添加自定义服务器,或使用预设快速开始使用流行的服务器。你也可以在此找到团队共享给你的服务器。
...
settings-mcp-list-section-shared-by-warp-and-team = 由 Kairos 和 { $name } 共享
settings-mcp-list-section-shared-by-warp-and-other-devices = 由 Kairos 和其他设备共享
settings-mcp-list-section-shared-from-warp = 来自 Kairos 的共享
...
settings-platform-description = 创建并管理 API 密钥,允许其他 Oz 云端 agent 访问你的 Kairos 账户。
    了解更多请访问
...
settings-platform-zero-state-description = 创建密钥以管理对 Kairos 的外部访问
settings-platform-create-api-key-description-personal = 此 API 密钥与你的用户绑定,可用于向你的 Kairos 账户发起请求。
settings-platform-create-api-key-description-team = 此 API 密钥与你的团队绑定,可用于代表你的团队发起请求。
settings-platform-create-api-key-name-placeholder = Kairos API 密钥
...
settings-referrals-page-title = 邀请朋友加入 Kairos
settings-referrals-anonymous-header = 注册以参与 Kairos 推荐计划
...
settings-referrals-reward-intro = 推荐朋友即可获得 Kairos 专属周边*
...
settings-warpify-page-title = SSH 集成
settings-warpify-description-prefix = 配置 Kairos 是否尝试对特定 Shell 进行集成(为其添加命令块、输入模式等支持)。
...
settings-warpify-section-ssh-subtitle = 在交互式 SSH 会话中启用 Kairos 集成功能。
...
settings-warpify-enable-ssh = 启用 SSH 集成
settings-warpify-install-ssh-extension = 安装 SSH 扩展
settings-warpify-install-ssh-extension-description = 控制远程主机未安装 Kairos 的 SSH 扩展时的安装行为。
settings-warpify-use-tmux = 使用 Tmux 集成
settings-warpify-tmux-description = tmux ssh 包装器在许多默认方式无效的场景下能正常工作,但可能需要你手动点击按钮才能集成。在新标签页中生效。
settings-warpify-ssh-tmux-toggle-binding-label = 用于集成的 SSH 会话检测

# --- ANCHOR-SUB-AI-PAGE (agent-settings-ai-page) ---
# 章节 / 副标题
settings-ai-warp-agent-header = Kairos 智能体
...
settings-ai-command-denylist-description = 匹配命令的正则表达式,Kairos 智能体执行这些命令前必须征得许可。
settings-ai-command-allowlist = 命令允许列表
settings-ai-command-allowlist-description = 匹配命令的正则表达式,Kairos 智能体可自动执行这些命令。
...
settings-ai-mcp-allowlist-description = 允许 Kairos 智能体调用这些 MCP 服务器。
settings-ai-mcp-denylist = MCP 拒绝列表
settings-ai-mcp-denylist-description = Kairos 智能体调用此列表中的任何 MCP 服务器前都必须征得许可。
settings-ai-info-banner-managed-by-workspace = 你的部分权限由工作区管理。

# 模型 / 配置
settings-ai-base-model = 基础模型
settings-ai-base-model-description = 此模型作为 Kairos 智能体背后的主要引擎,驱动大部分交互,并在需要时调用其他模型完成规划或代码生成等任务。Kairos 可能根据模型可用性自动切换备用模型,或将其用于会话摘要等辅助任务。
settings-ai-show-model-picker-in-prompt = 在提示中显示模型选择器
settings-ai-codebase-context = 代码库上下文
settings-ai-codebase-context-description = 允许 Kairos 智能体生成代码库的概要作为上下文。代码从不存储到我们的服务器。
...
settings-ai-rules-description = 规则帮助 Kairos 智能体遵循你的约定,无论是针对代码库还是特定工作流。
...
settings-ai-auto-show-rich-input-tooltip = 需要为你的编码智能体安装 Kairos 插件
...
settings-ai-cloud-agent-computer-use-description = 在 Kairos 应用中启动的 Cloud Agent 会话中启用计算机使用。
...
settings-ai-aws-bedrock-description = Kairos 加载并发送本地 AWS CLI 凭证以使用 Bedrock 支持的模型。
settings-ai-aws-bedrock-description-managed = Kairos 加载并发送本地 AWS CLI 凭证以使用 Bedrock 支持的模型。此设置由你的组织管理。
...
settings-features-ssh-wrapper = Kairos SSH 包装器
settings-features-receive-desktop-notifications = 接收来自 Kairos 的桌面通知
...
settings-features-make-default-terminal = 将 Kairos 设为默认终端
...
settings-features-default-terminal-current = Kairos 已是默认终端
...
settings-features-restart-warp-to-apply = 重启 Kairos 以使更改生效。
...
settings-code-codebase-index-description = Kairos 可在你浏览代码仓库时自动索引,帮助 agent 快速理解上下文并给出解决方案。代码从不存储到服务器。即使代码库无法被索引,Kairos 仍可通过 grep 与 find 工具调用浏览代码库并获取信息。
...
settings-code-auto-index-description = 启用后,Kairos 将在你浏览代码仓库时自动索引,帮助 agent 快速理解上下文并提供有针对性的解决方案。
...
settings-privacy-safe-mode-description = 启用此设置后,Kairos 会扫描 Block、Kairos Drive 对象内容以及 Oz 提示词中可能包含的敏感信息,并阻止将这些数据保存或发送到任何服务器。你可以通过正则表达式自定义匹配规则。
...
settings-privacy-telemetry-title = 帮助改进 Kairos
settings-privacy-telemetry-description = 应用分析数据帮助我们为你改进产品。我们可能会采集部分控制台交互数据,用于改进 Kairos 的 AI 能力。
...
settings-privacy-telemetry-docs-link = 详细了解 Kairos 如何使用数据
settings-privacy-data-management-title = 管理你的数据
settings-privacy-data-management-description = 你可以随时选择永久删除 Kairos 账户。删除后将无法继续使用 Kairos。
settings-privacy-data-management-link = 访问数据管理页面
settings-privacy-policy-title = 隐私政策
settings-privacy-policy-link = 阅读 Kairos 的隐私政策
...
settings-privacy-cloud-conv-description-on = 智能体对话可与他人共享,并在你于其它设备登录时保留。此数据仅用于产品功能,Kairos 不会将其用于分析。
...
settings-privacy-network-log-description = 我们构建了原生控制台,允许你查看 Kairos 与外部服务器的全部通信,让你确信工作始终安全。
...
settings-exec-profile-editor-base-model-desc = 该模型作为智能体的主要引擎,驱动绝大多数交互,并在需要时调用其他模型完成规划或代码生成等任务。Kairos 可能基于模型可用性或辅助任务(如对话摘要)自动切换到备选模型。
...
settings-exec-profile-editor-next-command-model-desc = 用于预测你下一条 shell 命令的模型(灰色行内自动建议 + 块结束后的零状态建议)。对延迟敏感——请选择你拥有的最小 / 最快 BYOE 模型。默认沿用基础模型。
...
settings-show-blocks-confirm-dialog-text = 确定要取消共享此命令块吗?
    ...
    取消后将无法通过链接访问,并将从 Kairos 服务器永久删除。
...
settings-appearance-custom-icon-restart-warning = 你可能需要重启 Kairos 才能让 macOS 应用所选图标样式。
...
settings-appearance-input-type-warp = Kairos
settings-appearance-input-type-shell = Shell (PS1)
settings-appearance-input-position-label = 输入位置
settings-appearance-input-mode-pinned-bottom = 固定在底部(Kairos 模式)
...
settings-environments-page-description = 环境定义了你的 ambient agent 在哪里运行。可通过 GitHub(推荐)、Kairos 辅助配置或手动配置在几分钟内创建一个。
...
settings-environments-section-team-default = 由 Kairos 和你的团队共享
settings-environments-section-team-named = 由 Kairos 和 { $team } 共享
...
quit-warning-title-app = 退出 Kairos?
...
# Manage Rules 页面(Kairos Drive 中的 AI Fact Collection)。
...
keybinding-desc-workspace-left-panel-warp-drive = 左侧面板:Kairos Drive
...
keybinding-desc-workspace-toggle-warp-drive = 切换 Kairos Drive
keybinding-desc-workspace-toggle-warp-drive-menu = Kairos Drive
...
keybinding-desc-workspace-search-drive = 搜索 Kairos Drive
...
keybinding-desc-workspace-terminate-app = 退出 Kairos
...
keybinding-desc-workspace-export-all-warp-drive-objects = 导出所有 Kairos Drive 对象
keybinding-desc-workspace-install-cli = 安装 Oz CLI 命令
keybinding-desc-workspace-uninstall-cli = 卸载 Oz CLI 命令

# AI 助手 / agent
keybinding-desc-workspace-toggle-ai-assistant = 切换 Kairos AI
...
keybinding-desc-workspace-show-settings-about-menu = 关于 Kairos
...
keybinding-desc-workspace-show-settings-warpify = 打开设置:SSH 集成
keybinding-desc-workspace-show-settings-warpify-menu = 配置 SSH 集成...
...
keybinding-desc-workspace-view-logs = 查看 Kairos 日志
...
keybinding-desc-workspace-init-project = 为 Kairos 初始化项目
...
keybinding-desc-workspace-install-opencode-warp-plugin = [调试] 安装 OpenCode Kairos 插件
keybinding-desc-workspace-use-local-opencode-warp-plugin = [调试] 使用本地 OpenCode Kairos 插件(仅测试用)
...
# 终端输入相关 binding
keybinding-desc-input-show-network-log = 显示 Kairos 网络日志
...
keybinding-desc-terminal-warpify-subshell = 集成子 shell
keybinding-desc-terminal-warpify-ssh-session = 集成 SSH 会话
...
keybinding-desc-ai-assistant-close = 关闭 Kairos AI
keybinding-desc-ai-assistant-focus-terminal-input = 从 Kairos AI 切回终端输入
keybinding-desc-ai-assistant-restart = 重启 Kairos AI
...
keybinding-desc-terminal-ask-ai-selection = 就所选内容询问 Kairos AI
keybinding-desc-terminal-ask-ai-last-block = 就最近的命令块询问 Kairos AI
keybinding-desc-terminal-ask-ai = 询问 Kairos AI
...
toggle-suffix-ssh-wrapper = Kairos SSH 包装器
...
settings-external-editor-choose-layout = 选择在 Kairos 中打开文件的布局
settings-external-editor-tabbed-header = 多个文件合并到同一编辑器面板
settings-external-editor-tabbed-desc = 开启后,同一标签页中打开的文件会自动归并到单一编辑器面板。
settings-external-editor-prefer-markdown = 默认用 Kairos Markdown 查看器打开 Markdown 文件
...
menu-block-open-in-warp = 在 Kairos 中打开
...
menu-block-ask-warp-ai = 询问 Kairos AI
...
menu-input-ask-warp-ai = 询问 Kairos AI
...
slash-cmd-open-skill-desc = 在 Kairos 内置编辑器中打开技能的 markdown 文件
slash-cmd-skills-desc = 调用技能
slash-cmd-add-prompt-desc = 添加新的智能体提示词
slash-cmd-add-rule-desc = 为智能体添加新的全局规则
slash-cmd-open-file-desc = 在 Kairos 代码编辑器中打开文件
...
prompt-editor-warp-prompt-section = Kairos 终端提示词
...
welcome-tips-command-palette-description = 无需双手离开键盘,即可轻松发现 Kairos 的全部功能。
...
welcome-tips-theme-picker-description = 选择内置主题,让 Kairos 更符合你的风格。也可以创建自己的主题。
...
search-filter-display-drive = Kairos Drive
...
drive-title = Kairos Drive
drive-create-team-text = 与团队成员共享命令和知识。
drive-join-team-header = 与已经在 Kairos 上的 { $count } 位团队成员协作。
...
workflow-ai-assist-tooltip = 使用 Kairos AI 生成标题、描述或参数
workflow-tooltip-restore-from-trash = 从废纸篓恢复 workflow
workflow-ai-assist-error-BYOE-required = 自动填充需要 BYOE 模型。请在“设置 → AI”中配置 provider 与模型。
...
workspace-left-panel-warp-drive = Kairos Drive
...
wasm-nux-open-desktop-title = 在 Kairos 桌面版中打开？
wasm-nux-open-desktop-detail = 之后的链接会自动在桌面版中打开。
wasm-nux-open-desktop-confirm = 在 Kairos 中打开
wasm-nux-download-title = 下载 Kairos 桌面版？
wasm-nux-download-description = Kairos 是一款内置 AI 和团队知识的智能终端。
wasm-nux-learn-more = 了解更多
wasm-nux-download-confirm = 下载
wasm-nux-object-kind-drive-objects = Kairos Drive 对象
wasm-nux-object-kind-shared-sessions = 共享会话
wasm-nux-object-kind-warp-links = Kairos 链接
...
auth-override-warning-confirm-title = 要删除个人 Kairos Drive 对象和偏好吗？
auth-override-warning-description = 看起来你刚刚通过网页浏览器登录了一个 Kairos 账号。如果继续，当前匿名会话中的所有个人 Kairos Drive 对象和偏好都将被永久删除。
...
auth-override-warning-accessibility-help = Kairos 检测到你通过网页浏览器进行了新的登录。按 Esc 键取消，并继续在未登录状态下使用 Kairos。
...
auth-needs-sso-link-detail = 点击下方按钮，将你的 Kairos 账号关联到你的 SSO 提供商。
...
auth-offline-first-use-description = 你当前处于离线状态。首次使用 Kairos 时需要联网。
auth-offline-first-use-learn-more = 了解更多
auth-offline-overlay-title = 离线使用 Kairos
auth-offline-overlay-paragraph-1 = Kairos 的所有非云端功能都可以离线使用。
auth-offline-overlay-paragraph-2 = 但在首次使用 Kairos 时，我们要求你保持联网，以启用 Kairos 的 AI 和云功能。
auth-offline-overlay-paragraph-3 = 我们向所有用户提供云功能，因此需要互联网连接来计量 AI 使用量、防止滥用，并将云对象与用户关联起来。如果你选择在退出登录状态下使用 Kairos，系统会把一个唯一 ID 绑定到匿名用户账号上，以支持这些功能。
auth-offline-overlay-dismiss = 关闭
auth-privacy-settings-title = 隐私设置
auth-privacy-settings-done = 完成
auth-privacy-settings-help-improve = 帮助改进 Kairos
auth-privacy-settings-help-improve-description = 高层级的功能使用数据可以帮助 Kairos 产品团队确定路线图优先级。
auth-privacy-settings-learn-more = 了解更多
auth-privacy-settings-send-crash-reports = 发送崩溃报告
auth-privacy-settings-crash-reports-description = 崩溃报告能帮助 Kairos 工程团队了解稳定性情况并改进性能。
...
auth-logout-unsynced-drive-objects-warning = 你有 { $count } 个未同步的 Kairos Drive 对象。退出登录后将失去这些对象。
...
cli-agent-plugin-codex-install-title = 为 Codex 启用 Kairos 通知
cli-agent-plugin-codex-install-subtitle = 将 Codex 更新到最新版本，然后启用聚焦时通知，这样你工作时 Kairos 就能显示通知。
...
cli-agent-plugin-deepseek-install-title = 为 DeepSeek 启用 Kairos 通知
...
cli-agent-plugin-claude-install-title = 安装 Claude Code 的 Kairos 插件
cli-agent-plugin-claude-install-subtitle = 确保你的机器已安装 jq，然后运行这些命令。
cli-agent-plugin-claude-add-marketplace-step = 添加 Kairos 插件市场仓库
cli-agent-plugin-install-warp-plugin-step = 安装 Kairos 插件
...
cli-agent-plugin-claude-update-title = 更新 Claude Code 的 Kairos 插件
...
cli-agent-plugin-gemini-install-title = 安装 Gemini CLI 的 Kairos 插件
cli-agent-plugin-gemini-run-command-restart = 运行以下命令，然后重启 Gemini CLI。
cli-agent-plugin-install-warp-extension-step = 安装 Kairos 扩展
cli-agent-plugin-gemini-restart-note = 重启 Gemini CLI 以启用插件。
cli-agent-plugin-gemini-update-title = 更新 Gemini CLI 的 Kairos 插件
cli-agent-plugin-update-warp-extension-step = 更新 Kairos 扩展
cli-agent-plugin-gemini-restart-update-note = 重启 Gemini CLI 以启用更新。
cli-agent-plugin-opencode-install-title = 安装 OpenCode 的 Kairos 插件
cli-agent-plugin-opencode-install-subtitle = 将 Kairos 插件加入 OpenCode 配置，然后重启 OpenCode。
...
cli-agent-plugin-opencode-update-title = 更新 OpenCode 的 Kairos 插件
...
terminal-bootstrapping-installing-warp-ssh-extension-progress = 正在安装 Kairos SSH 扩展... ({ $p }%)
terminal-bootstrapping-installing-warp-ssh-extension = 正在安装 Kairos SSH 扩展...
terminal-bootstrapping-updating-warp-ssh-extension = 正在更新 Kairos SSH 扩展...
...
agent-tip-command-palette = 按 <keybinding> 打开命令面板,访问 Kairos 操作和快捷方式。
agent-tip-warp-drive = 将可复用的工作流、Notebook 和提示词存入你的
agent-tip-redirect-running-agent = 输入新的提示词即可在智能体运行时重定向它。
agent-tip-add-context = 输入 `@` 将文件、块或 Kairos Drive 对象作为上下文添加到提示词。
...
agent-tip-action-warp-drive = Kairos Drive。
agent-tip-action-show-diff-view = 显示 diff 视图
agent-tip-voice-input = 按住 <keybinding>,直接用语音向智能体输入提示词。
hoa-welcome-banner-title = 引入通用智能体支持:用 Kairos 增强任何编码智能体
...
hoa-feature-native-code-review-description = 将 Kairos 代码审查中的行内评论直接发送到 Claude Code、Codex 或 OpenCode
resource-center-whats-new-section = 新变化
resource-center-getting-started-section = 入门
resource-center-maximize-warp-section = 充分使用 Kairos
...
resource-center-command-palette-description = 通过键盘访问 Kairos 的全部功能。
resource-center-set-theme-title = 设置主题
resource-center-set-theme-description = 选择主题,让 Kairos 符合你的偏好。
resource-center-custom-prompt-title = 使用自定义提示符
resource-center-custom-prompt-description = 设置 Kairos 以遵循你的 PS1 设置
resource-center-view-documentation = 查看文档
resource-center-integrate-ide-title = 将 Kairos 与 IDE 集成
resource-center-integrate-ide-description = 配置 Kairos,从你最常用的开发工具中启动
resource-center-how-warp-uses-warp-title = Kairos 如何使用 Kairos
resource-center-how-warp-uses-warp-description = 了解 Kairos 工程团队如何使用他们最喜欢的功能
...
onboarding-intention-title = 欢迎使用 Kairos
...
onboarding-ai-feature-warp-agents = Kairos 智能体
...
onboarding-agent-title = 自定义你的 Kairos 智能体
...
onboarding-agent-disable-warp-agent = 禁用 Kairos 智能体
...
onboarding-project-subtitle = 设置项目，让 Kairos 更适合编码。
...
onboarding-customize-title = 自定义你的 Kairos
...
onboarding-customize-warp-drive = Kairos Drive
onboarding-customize-tools-panel = 工具面板
onboarding-customize-code-review = 代码评审
onboarding-free-user-title = 开始使用吧。
onboarding-free-user-agent-title = 使用 Kairos 内置智能体进行开发
onboarding-free-user-agent-description = 使用 Oz（Kairos 内置智能体）迭代、规划和构建。可在本地或云端运行。
...
onboarding-free-user-subscribe-title = 订阅后即可使用 Kairos 的智能体开发能力。
...
onboarding-free-user-subscribe-item-drive = 不限量 Kairos Drive 对象和协作
...
auth-terms-prefix = 继续即表示你同意 Kairos 的{" "}
auth-terms-of-service = 服务条款
auth-log-in = 登录
auth-paste-token-from-browser = 点击这里粘贴浏览器中的 token
auth-login-slide-title-warp-drive = 开始使用 Kairos Drive
...
auth-disable-warp-drive = 停用 Kairos Drive
auth-disable-ai-features = 停用 AI 功能
auth-enable-warp-drive = 启用 Kairos Drive
...
auth-disable-warp-drive-confirm-title = 确定要停用 Kairos Drive 吗？
auth-disable-ai-features-confirm-title = 确定要停用 AI 功能吗？
auth-disable-warp-drive-confirm-body = Kairos Drive 可让你跨设备保存工作流和知识，并与团队分享。继续后，你将无法使用以下功能：
auth-disable-ai-features-confirm-body = 开启 AI 后 Kairos 会更强大。继续后，你将无法使用以下功能：
...
auth-require-login-ai-collaboration = 要使用 Kairos 的 AI 功能或与他人协作，请创建账号。
auth-require-login-drive-limit = 要在 Kairos Drive 中创建更多对象，请创建账号。
auth-require-login-share = 要分享内容，请创建账号。
auth-welcome-title = 欢迎使用 Kairos！
auth-sign-up-for-warp = 注册 Kairos
...
get-started-welcome-title = 欢迎使用 Kairos
...
terminal-banner-p10k-supported = Powerlevel10k 现在支持 Kairos！{"  "}
...
terminal-banner-pure-unsupported = Kairos 尚不支持 Pure。你可以考虑改用受支持的提示符。{"  "}
...
ai-footer-plugin-installed-restart-session = Kairos 插件已安装。请重启会话以启用。
ai-footer-installing-warp-plugin = 正在安装 Kairos 插件...
ai-footer-failed-install-warp-plugin = 安装 Kairos 插件失败
ai-footer-plugin-updated-restart-session = Kairos 插件已更新。请重启会话以启用。
ai-footer-updating-warp-plugin = 正在更新 Kairos 插件...
ai-footer-failed-update-warp-plugin = 更新 Kairos 插件失败
...
settings-import-reset-to-warp-defaults = 重置为 Kairos 默认设置
...
workspace-notification-permission-denied-toast = Kairos 没有发送桌面通知的权限。
workspace-troubleshoot-notifications-link = 排查通知问题
workspace-plan-synced-to-warp-drive-toast = 计划已同步到你的 Kairos Drive
workspace-remote-control-link-copied-toast = 远程控制链接已复制。
workspace-update-now = 立即更新
workspace-update-warp = 更新 Kairos
...
workspace-version-deprecation-without-permissions-banner = 若不立即更新，部分 Kairos 功能可能无法正常工作，但 Kairos 无法执行更新。
workspace-new-version-unable-to-update-banner = 有新版本可用，但 Kairos 无法执行更新。
workspace-unable-to-launch-new-installed-version = Kairos 无法启动新安装的版本。
...
terminal-warpify-without-tmux = 不使用 TMUX 进行集成
terminal-continue-without-warpification = 不进行集成继续
terminal-always-install = 始终安装
terminal-never-install = 从不安装
terminal-ssh-report-issue-prefix = 我们正在积极改进 Kairos 中 SSH 的稳定性。请考虑
...
terminal-agent-mode-setup-title = 为此代码库优化 Kairos？
...
terminal-shell-premature-subtext = 启动 { $shell_detail } 并对其进行集成时出错，导致进程终止。这里显示了集成脚本输出，可能能帮助定位原因。
...
notifications-banner-permissions-denied-title = Kairos 被拒绝发送通知的权限。
...
tooltip-secrets-not-sent-to-warp-server = *密钥不会发送到 Kairos 服务器。
