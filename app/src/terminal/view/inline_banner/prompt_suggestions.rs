...
pub struct PromptSuggestionBannerState {
    ...
    /// OpenWarp BYOE:模型主动调 `suggest_prompt` 工具产生的 chip 携带 action_id,
    /// accept/reject 时需要调 `complete_suggest_prompt_action` 关闭 oneshot channel
    /// 让 BYOE loop 拿到 result 继续下一轮。`None` 表示这条 chip 来自其他路径
    /// (例如 MAA 服务端被动建议),不需要走 BYOE 完成回写。
    pub BYOE_action_id: Option<crate::ai::agent::AIAgentActionId>,
}
