...
pub(super) async fn download_update_and_cleanup(
    ...
    // Kairos OSS: downloads installer to user's Downloads folder and highlights it in Explorer.
    // Does not run the Inno Setup installer silently; the user runs it manually.
...
pub(super) fn check_and_report_update_errors(ctx: &mut AppContext) {
    ...
    // Kairos OSS phase 3: cloud telemetry stripped. Autoupdate failures are logged locally
    // (the log file is preserved with a .log.reported rename) and visible to the user directly.
    #[cfg(feature = "crash_reporting")]
    {
        ...
        if error_count > 0 {
            log::error!(
                "Kairos: Windows auto-update log contains {error_count} error(s) (log: {:?})",
                log_path
            );
        }
        let _ = &contents;
...
fn app_name_prefix(channel: Channel) -> &'static str {
    match channel {
        ...
        Channel::Dev => "WarpDev",
        // Aligns with script/windows/bundle.ps1 OSS branch INSTALLER_NAME=KairosSetup,
        // so the GitHub Release asset KairosSetup.exe is correctly generated by installer_file_name().
        Channel::Oss => "Kairos",
    }
}

/// Kairos OSS channel download path: fetch the installer from a GitHub Release into Downloads,
/// then open Explorer highlighting the file. Does not run Inno Setup automatically.
async fn download_oss_to_downloads(client: &http_client::Client) -> Result<DownloadReady> {
    ...
    let asset = release.find_asset(&installer_name).with_context(|| {
        format!(
            "GitHub Release {} is missing asset {installer_name}; visit {} to download manually",
    ...
    if already_downloaded {
        log::info!(
            "Kairos installer already exists, skipping download: {} ({} bytes)",
            target_path.display(),
            asset.size
        );
    } else {
    ...
    if let Err(e) = Command::new("explorer")
        ...
        .spawn()
    {
        log::warn!("Failed to open Explorer (download already complete): {e:#}");
    }
